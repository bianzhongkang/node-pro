...
router.post('/addProduct', function(req, res, next) {
  let { images, brand, price } = req.body;
  sql.find('sh1811', 'product', { image }).then(data => {
    if(data.length == 0 && brand !== '' && image !== '' && price !== '') {
      sql.insert('sh1811', 'product', { image, brand, price }).then(() => {
...
router.post('/updateAction', function(req, res, next) {
  let { price, pageCode } = req.body;
  if( price !== '' ){
  	price = price * 1;
  	sql.update('sh1811', 'product', 'updateOne', { }, {$set: { price }}).then(() => {
...
// 导入用户
const usersxlsx = '/usr/local/node-pro/myapp/product.xlsx';
...
// 	sql.find('sh1811', 'product', { proname }).then(data => {
// 		sql.distinct('sh1811', 'product', 'proname').then(brandArr => {
// 			res.render('product', {
// 				activeIndex: 3,
// 				pageCode: 1,
// 				totalNumber: 1,
